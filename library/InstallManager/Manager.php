    ...
    public function checkTables()
    {
        ...
        foreach ($this->tables as $table) {
            if (Capsule::schema()->hasTable($table)) {
                $this->addMessage("'$table' tablosu mevcut.");
            }
        }
    ...
    public function checkRequirements()
    {
        ...
        foreach ($this->requirements as $path => $version)
        {
            if (! file_exists('library/' . $path . '.php')) {
                $this->addMessage("'$path' kutuphanesi bulunamadi.");
            } else {
                $class = '\\Library\\' . str_replace('/', '\\', $path);
                if ((float) $class::version() < (float) $version) {
                    $this->addMessage("'$path' kutuphane versiyonu uyusmuyor. -- $version / {$class::version()}");
                }
            }
        }
    }
